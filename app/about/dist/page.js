...
export default function AboutPage() {
  ...
  useEffect(() => {
    const fetchStaff = async () => {
      try {
        setIsLoading(true);
        // Using church-staff as requested
    ...
    <div className="bg-slate-50 min-h-screen pb-20 font-sans">
      ...
      <header className="bg-[#1a2233] text-white py-28 px-4 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          ...
          <p className="text-slate-300 max-w-2xl mx-auto text-lg leading-relaxed font-medium">
            ...
            community, and the rich traditions of the African Independent Pentecostal Church of Africa.
          </p>
        </div>
      </header>
        ...
        <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-100 p-10 md:p-16 grid grid-cols-1 md:grid-cols-2 gap-16">
          <div>
            ...
            <p className="text-slate-600 leading-relaxed text-lg">
              ...
              and serve the Kathelwa community with unwavering love and dedication.
            </p>
          </div>
          <div>
            ...
            <p className="text-slate-600 leading-relaxed text-lg">
              ...
              that fiercely loves God and transforms society through Christian values.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-slate-900 text-center">Church Leadership</h2>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
            {leaders.map((leader) => (
              <div key={leader.id} className="bg-white p-8 rounded-[2.5rem] border-2 border-transparent shadow-xl transition-all duration-500 hover:-translate-y-3 hover:border-orange-400 text-center">
                <div className="w-32 h-32 rounded-full mb-6 mx-auto border-4 border-slate-100 overflow-hidden shadow-inner">
                  <img 
                ...
                {/* Dynamically assigned colors based on role can be handled via a helper or simple conditional */}
                <p className="text-orange-600 text-xs font-black uppercase tracking-widest mb-4">{leader.role}</p>
                <p className="text-slate-500 text-sm">{leader.bio || "Serving with transparency and faith."}</p>
              </div>
            ))}
          </div>
        )}
      ...
      {/* CHOIRS */}
      <section className="bg-slate-900 text-white py-28 mt-24 rounded-[4rem] mx-4">
        <div className="max-w-7xl mx-auto px-8">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
            ...
            <Link href="/gallery?filter=Choir" className="bg-white/10 hover:bg-orange-600 px-8 py-4 rounded-full transition-all font-bold">
              View Choir Gallery
            </Link>
          </div>
